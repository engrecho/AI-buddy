...
**触发 \`download-video\` 的信号**（出现任意一个就走离线）：用户说"离线""下载""存到本地""保存下来""收藏到本地"，或发长视频，或上一轮已要求离线。
**默认行为：自动存入阅读列表（不下载）**。用户明确说"下载/存到本地/离线"才走 download-video。**不确定时主动询问用户**。下载保存路径由服务端统一配置，用户无需也无法在客户端配置。
