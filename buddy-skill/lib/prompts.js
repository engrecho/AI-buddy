...
1. 先检查 \`~/.all-platform-video-extract/scripts/\` 是否存在（也可用 \`GV_SKILL_DIR\` 环境变量指向其他位置）
2. **没装** → 提示用户并询问是否从 <https://github.com/engrecho/ExtractVideoSkill.git> 克隆到 \`~/.all-platform-video-extract/\`（用户同意后执行 \`git clone\`）
3. **已装** → 调用 ExtractVideoSkill：
   - 解析：\`node ~/.all-platform-video-extract/scripts/video_extract.cjs --json "<分享文本>"\`
   - 下载：\`node ~/.all-platform-video-extract/scripts/download_videos.cjs "<分享文本>"\`
4. 拿到 \`{title, host, vid, videoItemVoList, ...}\` 后，写入 Buddy 阅读列表：
   ...
   - **首次使用** download 脚本会引导用户填写保存地址（写入 \`~/.all-platform-video-extract/config.json\`）
...
- 社媒解析: 调 ExtractVideoSkill（\`~/.all-platform-video-extract\`），不依赖本 SKILL
`;
