---
name: buddy-skill
description: AI-Buddy 的官方 SKILL——让 AI 助手通过 API Key 查询、修改、整合用户在 AI-Buddy 中的任务、备忘、阅读收藏、随记。当用户提到"我的任务"、"添加任务"、"整理任务"、"记个备忘"、"把文章存下来"、"我有哪些 to do"、或发来抖音/B站/小红书/公众号等视频/图文分享链接或复制文本时使用。**社媒平台触发词**：抖音、快手、小红书、B站(bilibili)、公众号(微信)、微博、知乎、西瓜、YouTube、TikTok 等 1000+ 平台。**阅读/收藏类关键词**：阅读列表、收趣、收藏、收藏夹、存一下、帮我存、记下来、稍后读、把文章存下来。**保存规则**：非离线模式必须把 标题/链接/头图/摘要 四个字段全部写入【阅读】列表并自动打标签，保存后向用户汇报标题/链接/摘要/位置。非离线模式（默认）：只把 标题/链接/头图/摘要 写入【阅读】列表并自动打标签，绝不下载文件。仅当用户明确表达"离线保存/下载/存到本地/没网也能看/离线收藏"时才调用 download-video 命令——该命令把请求发给 AI-Buddy 服务端，由服务端完成下载并把文件存到服务端统一目录，Agent 自身绝不直接下载；尤其抖音/B站/小红书等视频链接默认绝不自动下载，以免占用服务端硬盘。本 SKILL 已内置社媒解析能力（1000+ 平台），无需安装任何外部 SKILL。
---
...
>
> **🟢 默认行为（绝大多数情况，非离线）**：用户发来社媒分享链接，只说"保存 / 收藏 / 存一下 / 收趣 / 记下来 / 稍后读"等 → **非离线**，仅 `extract-video` 解析元信息后写入【阅读】列表，**绝不下载文件**。尤其是抖音 / B站 / 小红书等**视频**链接，视频文件体积大，默认绝不自动下载，以免占用服务端硬盘。
> **🟡 离线仅在"明确"时**：仅当用户清晰表达"离线 / 下载 / 存到本地 / 没网也能看 / 离线收藏"等要把文件落到磁盘的意图时，才走 `download-video`（由服务端下载，文件存**服务端**）。每一条都单独判断是否明确要离线，不因上一条离线过就自动延续。
...
3. AI 调 `createReading(...)` 时把 `is_offline=true`、`offline_path=...` 带上；**同时仍建议补全 `title`/`url`/`cover_url`/`summary`/`tags`**，让离线条目在阅读列表里信息完整
