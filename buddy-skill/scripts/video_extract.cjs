 ...
 *
 * 依赖：仅 node 内置模块（crypto, fetch/undici, fs, path, os）
 * 注意：- 解析视频通常 3~15 秒，脚本默认超时 60s
 *       - /auth/keys 的公钥有效期约 5 分钟，超时需要重新拉取
 *       - cookie 自动从配置的解析服务获取，无需手动提供
 *       - 解析服务地址优先级：GV_HOST 环境变量 > ~/.all-platform-video-extract/config.json > 默认值
 */
...
// 解析服务地址：优先 GV_HOST 环境变量，其次配置文件，最后默认值
function loadHost() {
  if (process.env.GV_HOST) return process.env.GV_HOST.replace(/\/+$/, '') + '/';
  const cfgPath = path.join(os.homedir(), '.all-platform-video-extract', 'config.json');
  if (fs.existsSync(cfgPath)) {
    try {
      const cfg = JSON.parse(fs.readFileSync(cfgPath, 'utf8'));
      if (cfg && cfg.host) return String(cfg.host).replace(/\/+$/, '') + '/';
    } catch (_) { /* 配置文件损坏时忽略,使用默认值 */ }
  }
  return 'https://greenvideo.cc/';
}
