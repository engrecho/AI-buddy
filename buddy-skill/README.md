...
只要命中下列任一**触发词**或**关键词**，AI 就应按「解析 → 存【阅读】列表」流程处理。

**社媒平台触发词**（用户发来这些平台的分享链接 / 复制文本）：抖音、快手、西瓜、TikTok、B站（bilibili / b23.tv）、YouTube、微博、小红书、知乎、公众号（微信）等 1000+ 平台——凡以「分享链接 / 复制链接」形式出现的短视频、长视频、图文、公众号文章均命中。

**阅读 / 收藏类关键词**（用户想保存内容）：阅读列表、收趣、收藏、收藏夹、存一下、帮我存、记下来、稍后读、把文章存下来。
...
# 阅读收藏（非离线模式：标题/链接/头图/摘要 全部带上，并打标签）
node index.js add-reading \
  --url "https://..." \
  --title "好文标题" \
  --summary "一句话摘要（公众号/视频可自动生成）" \
...
**铁律：**
1. 离线下载永远走 `download-video`，**不要**直接 `node scripts/download_videos.cjs ...`。后者是服务端内部脚本，在 Agent 本机运行会把文件下到 Agent 机器，用户在 Buddy 网页看不到。注意：`scripts/download_videos.cjs` 是服务端 `server/extract.js` 下载接口的实际执行脚本（被 `BUDDY_SKILL_SCRIPTS` 软链目录引用），**属于仓库必要文件，不可删除**；它只是不应由 Agent 直接调用。
2. `download-video` 返回 `offline_path`，文件始终在服务端，与 Agent 在哪台机器无关。
3. 不需要、也不要安装外部 `ExtractVideoSkill`——buddy-skill 已完全自包含。

**默认行为 vs 例外：**
- 🟢 **默认（非离线）**：用户发来社媒链接只说「保存 / 收藏 / 存一下 / 收趣」等 → 仅 `extract-video` 解析元信息并写入【阅读】列表，**绝不下载文件**；尤其是抖音 / B站 / 小红书等视频链接，视频文件体积大，默认绝不自动下载，避免占用服务端硬盘。
- 🟡 **离线（仅明确时）**：仅当用户清晰表达「离线 / 下载 / 存到本地 / 没网也能看 / 离线收藏」等要把文件落到磁盘的意图时，才走 `download-video`，由服务端下载、文件存**服务端**。
- ⚪ **唯一例外**：仅当用户**明确说"把文件存到我自己的电脑/本地"**时，才可在用户本机下载一份本地副本；这属于用户本地副本，不应写入 AI-Buddy 的 `is_offline`/离线列表。
