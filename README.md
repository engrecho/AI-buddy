...
### v1.12.1 (2026-09-19)
- **修复**：还款计划自动生成在东八区服务器上日期整体偏移一天的时区 bug（toISOString→本地日期格式化），29~31 号还款日从此可正确录入；前端表单/回填/归还日期同步修复

### v1.12.0 (2026-09-19)
- **贷款详情页上半区重排**：新增顶部身份卡（来源/类型/还款方式）；基本信息卡精简为放款日/还款日+备注（最多3行可展开）；还款进度卡合并本金已还进度（已还本金/总本金加粗展示）；金额明细精简为已还金额（含本金+利息拆解）/待还金额/总利息；下一期待还卡补充还款日
- **还款详情改版**：表格改两行式列表；过期未标记期次自动记为"准时归还"（无需手动标记）；真实逾期可点击条目手动标记逾期/恢复归还；移除列表上的直接修改与撤销按钮
- **贷款列表页**：新增本月应还/已还/待还汇总条与逾期提醒；每笔贷款显示最近一次应还日期与金额
- **还款日支持 29~31 号**：校验放宽至 1~31，计划生成自动钳制到当月最后一天
